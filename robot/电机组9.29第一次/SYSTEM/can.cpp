...
void CAN2_Configuration(void)
{
    ...
    can2.CAN_TXFP = ENABLE;//发送FIFO优先级
	/*CAN位时序寄存器（CAN_BTR）*/
...
void CAN_SendCurrent(s16 current)
{
	CanTxMsg TxMessage;
	
	TxMessage.StdId = CAN_MOTOR_ID;
	TxMessage.IDE = CAN_Id_Standard;
	TxMessage.RTR = CAN_RTR_Data;
	TxMessage.DLC = 0x08;
	
	TxMessage.Data[0] = (u8)(current >> 8);
	TxMessage.Data[1] = (u8)current;
	TxMessage.Data[2] = 0;
	TxMessage.Data[3] = 0;
	TxMessage.Data[4] = 0;
	TxMessage.Data[5] = 0;
...
u16 GetEncoderNumber(CanRxMsg* pCanRxMsg)
{
	...
	EncoderNumber = pCanRxMsg->Data[0] << 8 | pCanRxMsg->Data[1];
...
{ 
	...
	pEncoder->Diff=pEncoder->RawValue-pEncoder->PreRawValue;
	         if( pEncoder->Diff < -4000 ) //正转过零
            {pEncoder->SumValue+=(pEncoder->Diff+8192);}
							else if(pEncoder->Diff>4000)
							{pEncoder->SumValue+=(pEncoder->Diff-8192);}
							else{pEncoder->SumValue+=pEncoder->Diff;}
